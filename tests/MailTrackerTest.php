...
use valsis\MailTracker\MailTracker;
...
class AddressVerificationTest extends TestCase
{
    ...
    {
        return ['valsis\MailTracker\MailTrackerServiceProvider'];
    }
    ...
    public function testSendMessage()
    {
        ...
        $old_email = \valsis\MailTracker\Model\SentEmail::create([
                'hash' => Str::random(32),
            ]);
        $old_url = \valsis\MailTracker\Model\SentEmailUrlClicked::create([
        ...
        Event::assertDispatched(valsis\MailTracker\Events\EmailSentEvent::class);
    ...
    {
        $track = \valsis\MailTracker\Model\SentEmail::create([
                'hash' => Str::random(32),
            ]);
        ...
        $url = action('\valsis\MailTracker\MailTrackerController@getT', [$track->hash]);
        $this->visit($url);

        $track = $track->fresh();
        $this->assertEquals($pings, $track->opens);

        Event::assertDispatched(valsis\MailTracker\Events\ViewEmailEvent::class);
    }
    ...
    public function testLink()
    {
        ...
        $url = action('\valsis\MailTracker\MailTrackerController@getL', [
                \valsis\MailTracker\MailTracker::hash_url($redirect), // Replace slash with dollar sign
                $track->hash
            ]);
        $this->call('GET', $url);
        $this->assertRedirectedTo($redirect);

        Event::assertDispatched(valsis\MailTracker\Events\LinkClickedEvent::class);
    ...
    public function random_string_in_link_does_not_crash(Type $var = null)
    {
        ...
        try {
            $this->call('GET', $url);
        } catch (valsis\MailTracker\Exceptions\BadUrlLink $e) {
    ...
    public function it_retrieves_the_mesage_id_from_ses()
    {
        ...
        $tracker = new \valsis\MailTracker\MailTracker();

        $tracker->beforeSendPerformed($event);
        $tracker->sendPerformed($event);

        $sent_email = \valsis\MailTracker\Model\SentEmail::orderBy('id', 'desc')->first();
    ...
    {
        $url = action('\valsis\MailTracker\SNSController@callback');
        $this->post($url, [
                'message' => json_encode([
                        // Required
    ...
    public function it_processes_a_delivery()
    {
        ...
        $this->post(action('\valsis\MailTracker\SNSController@callback'), [
                'message' => json_encode([
                        // Required
                        'Message' => json_encode([
    ...
    public function it_processes_a_bounce()
    {
        ...
        Event::assertDispatched(valsis\MailTracker\Events\PermanentBouncedMessageEvent::class, function ($event) {
    ...
    public function it_handles_ampersands_in_links()
    {
        ...
        $track = \valsis\MailTracker\Model\SentEmail::whereHash($hash)->first();
    ...
    public function it_retrieves_header_data()
    {
        ...
        $track = \valsis\MailTracker\Model\SentEmail::orderBy('id', 'desc')->first();
    ...
    public function it_can_retrieve_url_clicks_from_eloquent()
    {
        ...
        $urlClick = \valsis\MailTracker\Model\SentEmailUrlClicked::create([
        ...
        $this->assertInstanceOf(\valsis\MailTracker\Model\SentEmailUrlClicked::class, $track->urlClicks->first());
    }
}
